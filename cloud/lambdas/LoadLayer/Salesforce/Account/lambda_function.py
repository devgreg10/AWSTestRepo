...
from data_core.salesforce.account.sf_account_db_helper import SalesforceAccountDbHelper
from data_core.salesforce.account.sf_account_db_models import SfAccountSourceModel
...
def lambda_handler(event, context):
    ...
    function_name = "Load layer lambda for Salesforce Account"
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                for line in response['Body'].iter_lines():
                    ...
                    if line.strip():  # Skip empty lines
                        ...
                        account_json = json.loads(line)

                        # Create the SfAccountSourceModel object from the dictionary
                        sf_source_account = SfAccountSourceModel(**account_json)

                        # logging.info(f"SfAccountSourceModel: {json.dumps(asdict(sf_source_account), indent=4)}")

                        # Add the account to the current chunk
                        chunk.append(sf_source_account)

                        # Once the commit_batch_size is met, process the chunk and reset the list
                        if len(chunk) == commit_batch_size:
                            
                            logging.info(f"{function_name} - calling SalesforceAccountDbHelper.insert_sf_raw_accounts_from_source_accounts for chunk size {commit_batch_size}")
                            SalesforceAccountDbHelper.insert_sf_raw_accounts_from_source_accounts(
                                db_connection = db_connection,
                                source_accounts = chunk,
                                commit_changes = True)
                                
                            # reset chunk upon commit
                            chunk = []
                
                # Process any remaining accounts that didn't fill the last chunk and close the DB connection
                if chunk:
                    logging.info(f"{function_name} - calling SalesforceAccountDbHelper.insert_sf_raw_accounts_from_source_accounts for chunk size {len(chunk)}")
                    SalesforceAccountDbHelper.insert_sf_raw_accounts_from_source_accounts(
                        db_connection = db_connection,
                        source_accounts = chunk,
                        commit_changes = True)              
            ...
            logger.error(f"{function_name} - Error occurred loading S3 accounts to Raw: {e}")
            if db_connection:
