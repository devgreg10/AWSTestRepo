...
from data_core.salesforce.listing_session.sf_listing_session_db_helper import SalesforceListingSessionDbHelper
from data_core.salesforce.listing_session.sf_listing_session_db_models import SfListingSessionSourceModel
...
def lambda_handler(event, context):
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                for line in response['Body'].iter_lines():
                    ...
                    if line.strip():  # Skip empty lines
                        ...
                        listing_session_json = json.loads(line)

                        # Create the SfListingSessionSourceModel object from the dictionary
                        sf_source_listing_session = SfListingSessionSourceModel(**listing_session_json)

                        # logging.info(f"SfListingSessionSourceModel: {json.dumps(asdict(sf_source_listing_session), indent=4)}")

                        # Add the listing_session to the current chunk
                        chunk.append(sf_source_listing_session)

                        # Once the commit_batch_size is met, process the chunk and reset the list
                        if len(chunk) == commit_batch_size:
                            
                            logging.info(f"{function_name} -calling SalesforceListingSessionDbHelper.insert_sf_raw_listing_sessions_from_source_listing_sessions for chunk size {commit_batch_size}")
                            SalesforceListingSessionDbHelper.insert_sf_raw_listing_sessions_from_source_listing_sessions(
                                db_connection = db_connection,
                                source_listing_sessions = chunk,
                                commit_changes = True)
                                
                            # reset chunk upon commit
                            chunk = []
                
                # Process any remaining listing_sessions that didn't fill the last chunk and close the DB connection
                if chunk:
                    logging.info(f"{function_name} - calling SalesforceListingSessionDbHelper.insert_sf_raw_listing_sessions_from_source_listing_sessions for chunk size {len(chunk)}")
                    SalesforceListingSessionDbHelper.insert_sf_raw_listing_sessions_from_source_listing_sessions(
                        db_connection = db_connection,
                        source_listing_sessions = chunk,
                        commit_changes = True)              
            ...
            logger.error(f"{function_name} - Error occurred loading S3 listing_sessions to Raw: {e}")
            if db_connection:
