...
class SalesforceAccountDbHelper:
    ...
    def insert_sf_raw_accounts(*, 
        ...
        Args:
        - accounts: List of SfAccountRawDbModel instances.
        ...
        INSERT INTO ft_ds_raw.sf_account (
            ...
            %(chapter_administrator__c)s,
            %(chapter_affiliation__c)s,
            %(territory__c)s,
            %(chapter_id__c)s,
            %(membership_offered__c)s,
            %(payments_accepted_in_person__c)s,
            %(time_zone__c)s,
            %(coach_retention_rate__c)s,
            %(count_account__c)s,
            %(current_coach_retention_percentage__c)s,
            %(performance_record_type__c)s,
            %(ys_report_chapter_affiliation__c)s,
            %(insurance_expires__c)s,
            %(affiliate_delivery_partner__c)s,
            %(date_joined__c)s,
            %(equipment__c)s,
            %(graduate__c)s,
            %(home_school__c)s,
            %(kindergarten__c)s,
            %(account_id__c)s,
            %(additional_trade_namechap_affiliation__c)s,
            %(additional_trade_name_logo__c)s,
            %(parent_account_id_18__c)s,
            %(account_inactive_date__c)s,
            %(legacy_id__c)s,
            %(ein__c)s,
            %(governance_structure__c)s,
            %(youth_population__c)s,
            %(contract_effective_date__c)s,
            %(active__c)s,
            %(chapter_membership_price__c)s,
            %(chapter_standing__c)s,
            %(contract_expiration_date__c)s,
            %(contract_status__c)s,
            %(county__c)s,
            %(ho_contracted_location__c)s,
            %(inactive_date__c)s,
            %(legal_entity_name__c)s,
            %(membership_active__c)s,
            %(membership_discount_amount__c)s,
            %(membership_discount_percentage__c)s,
            %(membership_end_date__c)s,
            %(membership_start_date__c)s,
            %(military_discount_amount__c)s,
            %(military_discount_percentage__c)s,
            %(primary_contact_email__c)s,
            %(primary_contact__c)s,
            %(primary_contact_email_address__c)s,
            %(program_location_key__c)s,
            %(program_location_type__c)s,
            %(reggie_account_id__c)s,
            %(reggie_id__c)s,
            %(reggie_location_id__c)s,
            %(reggie_name__c)s,
            %(region__c)s,
            %(secondary_contact__c)s,
            %(location_type__c)s,
            %(sibling_discount_amount__c)s,
            %(sibling_discount_percentage__c)s,
            %(service_area__c)s,
            %(insurance_expiration_date__c)s,
            %(former_trade_names__c)s,
            %(former_legal_entity__c)s,
            %(organization_city__c)s,
            %(organization_state__c)s,
            %(partner_program_type__c)s,
            %(pre_school__c)s,
            %(status__c)s,
            %(title_i__c)s,
            %(enrollment__c)s,
            ...
            %(dcr__c)s,
            %(owner_id__c)s,
            %(program_director__c)s,
            %(executive_director__c)s,
            %(board_chair__c)s,
            %(mdr_pid__c)s,
            %(partner_program_type_validation_check__c)s,
            %(duplicates_removed__c)s,
            %(nces_id__c)s,
            %(currency__c)s,
            %(territory_partner_acct__c)s,
            %(chapter_country__c)s,
            %(financial_aid_active__c)s,
            %(new_parent_registration__c)s,
            %(additional_trade_name__c)s,
            %(ft_app_pilot__c)s,
            %(international_chapter__c)s,
            %(customer_id__c)s,
            %(award_badges_enabled__c)s,
            %(peer_group__c)s,
            %(in_market_partners__c)s,
            %(how_many_holes__c)s,
            %(if_other_fill_in_how_many__c)s,
            %(golf_course_type__c)s,
            %(please_describe_discount__c)s,
            %(chapter_owns_this_facility__c)s,
            %(if_yes_how_long_is_the_lease__c)s,
