...
from data_core.salesforce.session_registration.sf_session_registration_db_helper import SalesforceSessionRegistrationDbHelper
from data_core.salesforce.session_registration.sf_session_registration_db_models import SfSessionRegistrationSourceModel
...
def lambda_handler(event, context):
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                for line in response['Body'].iter_lines():
                    ...
                    if line.strip():  # Skip empty lines
                        ...
                        session_registration_json = json.loads(line)

                        # Create the SfSessionRegistrationSourceModel object from the dictionary
                        sf_source_session_registration = SfSessionRegistrationSourceModel(**session_registration_json)

                        # logging.info(f"SfSessionRegistrationSourceModel: {json.dumps(asdict(sf_source_session_registration), indent=4)}")

                        # Add the session_registration to the current chunk
                        chunk.append(sf_source_session_registration)

                        # Once the commit_batch_size is met, process the chunk and reset the list
                        if len(chunk) == commit_batch_size:
                            
                            logging.info(f"{function_name} - calling SalesforceSessionRegistrationDbHelper.insert_sf_raw_session_registrations_from_source_session_registrations for chunk size {commit_batch_size}")
                            SalesforceSessionRegistrationDbHelper.insert_sf_raw_session_registrations_from_source_session_registrations(
                                db_connection = db_connection,
                                source_session_registrations = chunk,
                                commit_changes = True)
                                
                            # reset chunk upon commit
                            chunk = []
                
                # Process any remaining session_registrations that didn't fill the last chunk and close the DB connection
                if chunk:
                    logging.info(f"{function_name} - calling SalesforceSessionRegistrationDbHelper.insert_sf_raw_session_registrations_from_source_session_registrations for chunk size {len(chunk)}")
                    SalesforceSessionRegistrationDbHelper.insert_sf_raw_session_registrations_from_source_session_registrations(
                        db_connection = db_connection,
                        source_session_registrations = chunk,
                        commit_changes = True)              
            ...
            logger.error(f"{function_name} - Error occurred loading S3 session_registrations to Raw: {e}")
            if db_connection:
