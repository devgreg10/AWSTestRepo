...
BEGIN
    ...
    --ensures that only the most recent version of the reference data is moved into the valid zone
    AND "Upload date" = (SELECT max("Upload date") from ft_ds_raw.service_area_diversity_reference_data)
    ;
    ...
    --if we ever swap the service_area_diversity_reference_data table to also include versions, then this step will need to be removed
