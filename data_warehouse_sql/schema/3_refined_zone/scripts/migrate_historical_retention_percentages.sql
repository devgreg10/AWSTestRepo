...
--This part of the script is for migrating the retention metric calculation to be precise to 3 digits as well as change over the methodology
-- save the data currently in the table to a temp table
create temp table temp_retention_3_precision_upgrade as
select * from ft_ds_refined.metric_historical_retention_percentage;

--drop downstream views
drop view ft_ds_refined.metric_historical_daily_retention_percentage_view;
...
CREATE TABLE IF NOT EXISTS ft_ds_refined.metric_historical_retention_percentage (
    ...
    metric_calc_date TIMESTAMPTZ,
    chapter_id CHAR(18),
    retention_percentage NUMERIC(5,3),
    eoy_indicator VARCHAR(20)
);

--re-insert the old data into the newly defined table
insert into ft_ds_refined.metric_historical_retention_percentage
select * from temp_retention_3_precision_upgrade;

--get rid of  all of the previoulsy improperly calculated historical calculations
delete from ft_ds_refined.metric_historical_retention_percentage
...
--execite that function to get this year's retention
select ft_ds_admin.calculate_historical_retention_percentage();

--verify that the counts look correct
select * from ft_ds_refined.metric_historical_retention_percentage mhrp order by metric_calc_date desc;